...
async function main() {
  ...
  // 1. Clear existing flavors
  await prisma.flavor.deleteMany();
  console.log('🗑️  Cleared existing flavors from SQL');
  ...
  // 3. Create Admin User
  const adminEmail = process.env.ADMIN_EMAIL || 'saikumar89515@gmail.com';
  ...
  await prisma.user.upsert({
    ...
    create: {
      email: adminEmail,
      password: adminPassword, // Note: In real app, this should be hashed. For demo, we'll hash later.
      name: 'Admin Saikumar',
      role: 'admin'
    }
  });

  console.log(`🍦 Successfully seeded ${products.length} flavors into SQL!`);
